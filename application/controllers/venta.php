 ...
 proveedor.proveedor_nombre,proveedor.id_proveedor";
    ...
    function lst_reg_pagospendientes()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->load->view('menu/ventas/tbl_listareg_pagospendiente');
        } else {
            redirect(base_url() . 'venta/', 'refresh');
        }
    }
    ...
    function lst_reg_pagospendientes_json()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $join = array('credito', 'cliente', 'documento_venta', 'condiciones_pago', 'consolidado_detalle', 'usuario');
            $campos_join = array('credito.id_venta=venta.venta_id', 'cliente.id_cliente=venta.id_cliente',
                'documento_venta.id_tipo_documento=venta.numero_documento', 'condiciones_pago.id_condiciones=venta.condicion_pago', 'consolidado_detalle.pedido_id=venta.venta_id', 'usuario.nUsuCodigo=venta.id_vendedor');
            $tipo_join = array('left', null, null, null, 'left', null);
            ...
            if (count($lstVenta) > 0) {
                ...
                foreach ($lstVenta as $v) {
                    ...
                    if (floatval($pendiente != 0.00)) {
                        $botonas .= '<a onclick="pagar_venta(\'' . $v['venta_id'] . '\')" class="btn btn-default tip" title="Pagar"><i class="fa fa-paypal"></i> Pagar</a>' .
                            '<a class="btn btn-default tip" title="Ver Venta"  onclick="nota_credito(\'' . $v['venta_id'] . '\',\'' . $v['documento_Serie'] . '\',\'' . $v['documento_Numero'] . '\')"><i class="fa fa-file-archive-o"></i> Nota de Cr&eacute;dito</a>';
                    }
                    $botonas .= '</div>';
                    $PRODUCTOjson[] = $botonas;
                    $array['productosjson'][] = $PRODUCTOjson;
